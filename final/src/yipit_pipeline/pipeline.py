...
def validate_outputs(output_dir: Path) -> Dict[str, int]:
    ...
    embedding_index = pd.read_csv(output_dir / "article_embedding_index.csv")
    embedding_manifest = json.loads(
        (output_dir / "embedding_manifest.json").read_text(encoding="utf-8")
    )
    ...
    if embedding_index["article_id"].astype(str).tolist() != articles["article_id"].astype(str).tolist():
        raise ValueError("article embedding index and fact_article are not aligned")
    if embedding_index["embedding_row"].tolist() != list(range(len(articles))):
        raise ValueError("article embedding index rows are not contiguous and ordered")
    if embeddings.ndim != 2 or embeddings.shape[1] != int(embedding_manifest["dimension"]):
        raise ValueError("embedding matrix dimension does not match embedding manifest")
    return {
        "dim_company": len(dim),
...
def run_pipeline(
    ...
    """Run all transformations, validate them in staging, and publish the outputs."""
