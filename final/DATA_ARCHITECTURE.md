...
| `record_hash` | Hash of metadata business attributes for change detection |
...
3. Select the highest `top_k` scores using a stable descending sort. This is simple and deterministic for the supplied dataset size.
...
1. Read inputs and build every output completely in a temporary run directory.
2. Validate the complete staged dataset, including row counts, uniqueness, foreign keys, required exports, and embedding integrity.
3. Replace each published artifact with its validated staged version using an atomic file-level rename.
4. Validate the published output again and record the validated row counts in the run manifest.

Transformation or staged-validation failures leave the previous published output unchanged. The local filesystem publication loop provides atomic replacement per artifact, but it is not a transaction across the entire output collection. A production implementation should publish a versioned directory or object-store prefix and atomically update a current-version pointer after validation.
...
- Every ARR fact has a positive integer `arr_usd`; missing or invalid dates must be explicitly marked with `observation_status = 'UNDATED'`.
- Published output files were written and can be read back.
- Embedding count equals embedded article count, and the embedding-index article IDs exactly match `fact_article` order.
...
The search command requires exactly one of `--query` or `--article-id`. The standard `run` command regenerates every required modeled CSV, `ai_articles_enriched.csv`, and the reusable embedding artifacts.
