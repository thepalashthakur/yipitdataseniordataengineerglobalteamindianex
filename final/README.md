...
The design and operational reasoning are documented in [DATA_ARCHITECTURE.md](DATA_ARCHITECTURE.md), the required Data Architecture Document.
...
Detailed business rules, including currency conversion, date ambiguity, company aliases, taxonomy mappings, idempotency, backfills, and schema evolution, are in [DATA_ARCHITECTURE.md](DATA_ARCHITECTURE.md).
...
`pipeline_run.json` records source checksums and transformation/configuration versions. Production backfills and schema-evolution handling are described in [DATA_ARCHITECTURE.md](DATA_ARCHITECTURE.md).
